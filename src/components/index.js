import MyButton from './MyButton';
import MyInput from './MyInput';
import MyGap from './MyGap';
import MyHeader from './MyHeader';
import MyPicker from './MyPicker';
import MyCalendar from './MyCalendar';
import BottomNavigator from './BottomNavigator'
import MyRadio from './MyRadio';
import MyInputSecond from './MyInputSecond';
import MyButtonSecond from './MyButtonSecond';
import MyCalendarSecond from './MyCalendarSecond';
import MyInputLogin from './MyInputLogin';
import MyImageUpload from './MyImageUpload';
import MyTimePicker from './MyTimePicker';
import MyFileUploader from './MyFileUpload';
import FileUpload from './FileUpload';
import MySecondPicker from './MySecondPicker';
import MyInputLoss from './MyInputLoss';
import MyPickerLoss from './MyPickerLoss';





export { MyGap, MyButton, MyInput, MyHeader, MyPicker, MyCalendar, BottomNavigator, 
    MyRadio,MyInputSecond,MyButtonSecond ,MyCalendarSecond, MyInputLogin,MyImageUpload,
     MyTimePicker,MyFileUploader, FileUpload, MySecondPicker, MyInputLoss,MyPickerLoss};
